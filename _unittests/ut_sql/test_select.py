...
class TestSelect (unittest.TestCase):
    ...
    def test_select_simple_square(self) :
        ...
        fLOG(";".join([str(_) for _ in iter2.Schema]))
        fLOG(l)
        if l != [{'age4': 50.0, 'nom': 'nom'}, {'age4': 800.0, 'nom': 'jean'}, {'age4': 2.0, 'nom': 'jeanne'}] :
            raise Exception(str(l))
    ...
    def test_select_mismatch(self) :
        ...
        try :
            ...
            raise TypeError("we should not be able to reach this code due to confusion between iter and tbl")
    ...
    def test_select_operators(self) :
        ...
        exp = [{'age0': 10, 'ages': 9.5, 'agemod': 1, 'agem': 5.0, 'agep': 3.1622776601683795, 'agea': 10.5, 'agedd': 3, 'nom': 'nom'},
               {'age0': 40, 'ages': 39.5, 'agemod': 1, 'agem': 20.0, 'agep': 6.324555320336759, 'agea': 40.5, 'agedd': 13, 'nom': 'jean'},
    ...
    def test_where_or(self) :
        ...
        schema = [ ("nom", str), ("age", int) ]
        tbl = IterRow (schema, l)
        iter = tbl.where( ( tbl.age == 2).Or (tbl.age == 40), append_condition = True)
        res = list(iter)
